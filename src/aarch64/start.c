...
void secondary_boot(void)
{
    ...
    if (cpu_id == 1)
    {
        if (async_log) {
            ...
            /* POC43: CPU1 becomes the Framethrower RGB565->I420 producer. */
            extern void emu68_uvc_cpu1_worker(void);
            emu68_uvc_cpu1_worker();
        }
#endif
    }
    else if (cpu_id == 2)
    ...
    {
        /* POC43: preserve POC41 sole ownership of DWC2/UVC on CPU3. */
        extern void emu68_uvc_cpu3_worker(void);
        emu68_uvc_cpu3_worker();
    }
#endif
#else
    (void)async_log;
